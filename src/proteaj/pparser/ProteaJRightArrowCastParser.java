...
public class ProteaJRightArrowCastParser extends PackratParser<CastExpression> {
  ...
  public static ProteaJRightArrowCastParser getParser(CtClass type) {
    if (! parsers.containsKey(type)) parsers.put(type, new ProteaJRightArrowCastParser(type));
    return parsers.get(type);
  }

  private static Map<CtClass, ProteaJRightArrowCastParser> parsers = new HashMap<CtClass, ProteaJRightArrowCastParser>();

  private ProteaJRightArrowCastParser(CtClass type) { this.type = type; }
